...
public class SensoryRecognitionPattern implements Cloneable {
	...
	private Map<SensoryAttribute<?>, Certainty> recognizers = new HashMap<>(MAX_IDENTIFIERS, 1);
	 ...
	 * @return
	 */
	public Certainty recognizes(SensoryInput input, boolean actual, Mind sensor) {
		int counter = 0;
		for (SensoryAttribute<?> att : recognizers.keySet()) {
			...
			if (this.recognizers.get(att) == Certainty.COMPLETELY_CERTAIN && mat) {
				return Certainty.COMPLETELY_CERTAIN;
			} else if (mat) {
				counter++;
			}
		}
	...
	@Override
	public SensoryRecognitionPattern clone() {
	 ...
	 * @return
	 */
	public Certainty getRecognizability(SensoryAttribute<?> forTrait) {
	 ...
	 * @return
	 */
	public Certainty remove(IKnowledgeType<?> trait) {
	...
	public SensoryRecognitionPattern add(SensoryAttribute<?> trait, boolean force) {
		...
		if (trait.getRecognizability() == Certainty.FALSE) {
	 ...
	 * @return
	 */
	public SensoryRecognitionPattern changeRecognition(SensoryAttribute<?> trait, Certainty level) {
