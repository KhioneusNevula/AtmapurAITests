...
import psych_first.action.goal.Task.ActionWrapper.PossibleActionWrapper;
import psych_first.action.types.Action;
import psych_first.actionstates.checks.IsKnownCheck;
...
public class Task {
	...
	private int maxAttempts = 1; // TODO make this instance-specific
	...
	public Task(Goal goal, Task deeperTask, int maxActions) {
		if (goal.isEnd() || goal.getPriority() == Priority.X_PRIORITY)
			throw new IllegalArgumentException("Goal {" + goal + "} \ncannot be " + (goal.isEnd() ? "an end-type" : "")
					+ (goal.getPriority() == Priority.X_PRIORITY ? " no-priority" : "") + " goal.");
		this.goal = goal;
	 ...
	 * @return
	 */
	public int getPlanAttempts() {
		return planAttempts;
	}

	public void oneMorePlanAttempt() {
		this.planAttempts++;
	}

	public void oneMoreActionAttempt() {
		this.currentActionAttempts++;
	}

	public boolean isTooManyActionAttempts() {
		return this.currentActionAttempts > this.maxAttempts;
	}

	public void refreshPlanAttempts() {
		this.planAttempts = 0;
	}

	public void refreshActionAttempts() {
		this.currentActionAttempts = 0;
	}

	public int getActionAttempts() {
		return currentActionAttempts;
	}
	...
	public String report() {
		...
		str.append("}").append(",plan_attempts:" + planAttempts).append(",action_attempts:" + currentActionAttempts)
				.append("," + (wasRecentActionSuccessful ? "" : "un") + "successfulRecentAction:" + this.recentAction);
		return str.append("\n}").toString();
	}

	@Override
	public String toString() {
		return "Task{goal:" + goal + ",attempts:" + planAttempts + ",actionCount:" + this.actions.size()
	 ...
	 * e.g. conditions change or too many attempts have been tried <br>
	 * TODO actually complete the action execution part of the mind
	 * 
	 * @param will
	 * @return
	 */
	public ActionWrapper startExecutingAction(Will will) {
		...
		this.oneMoreActionAttempt();
		if (this.isTooManyActionAttempts()) {
			return null;
		}
		if (this.actions.peek().action._canExecute(will.getMind().getOwner(), will, this, actions.peek().requirement,
			...
			System.err.println(will.getMind() + " unable to execute " + actions.peek().getAction() + " due to \""
					+ actions.peek().getAction().interpretExecutionProblem(actions.peek().requirement, result,
							this.getExecutionInformation(actions.peek().getAction()), will.getMind().getCulture())
					+ "\"");
	...
	/**
	 * purges data stored by this action
	 * 
	 * @param a
	 */
	public void purgeAction(Action a) {
		this.executionInformation.remove(a);
		this.executionTicks.remove(a);
	}
	...
	public boolean willBeComplete() {
		if (actions.isEmpty())
			return false;
		return this.actions.peek().requirement.isEmpty() || this.actions.peek().requirement.isComplete();
	}
	 ...
	 * or it is too full to generate a plan; return the previous requirement wrapper
	 * if the task is complete already
	 * 
	 * @param clear = whether to clear any previous action plan
	 * 
	 * @return
	 */
	public RequirementWrapper generateActionPlan(Mind mind, int inefficiency, boolean clear) {
		Iterable<Action> possible = mind.getPossibleActions();
		oneMorePlanAttempt();
		if (planAttempts > maxAttempts) {
			return null;
		}
		if (clear) {
			this.clearActions();
		} else if (this.isTooFull()) {
			return null;
		}
		if (this.getNextGoal().isEmpty() || this.getNextGoal().goalUpdate(mind)) {
			return this.getNextGoal().getRequirementWrapper();
		}
		...
		if (reqs.isMulti() || reqs.isEmpty())
			return reqs;
		while (canAddNewActions() && !(this.getNextGoal().isEmpty() || this.getNextGoal().goalUpdate(mind))) {
	...
	/**
	 * does not try to resolve profiles/eliminate conditions for profiles with no
	 * conditions or ones which explicitly are not to be resolved
	 */
		...
		for (RequirementGoal req : reqs) {
			for (ProfileType ptype : ProfileType.values()) {
				...
				if (ptype == ProfileType.USER) { // if user profile, only check if the actor themself satisfies it
					...
					if (req.getState().getFor(ptype).conditionsUnfulfilledBy(user, mind.getCulture()).isEmpty()) {
						req.getState().getProfile(ptype).resolve(user);
						req.getState().eliminateResolvedConditions(req.getState().getProfile(ptype), mind.getCulture());
					}
					...
					if (req.getState().getProfile(ptype).isResolved()) {
						...
						if (req.getState().getFor(ptype).conditionsUnfulfilledBy(already, mind.getCulture())
								.isEmpty()) {
							res = already;
						}
					}
					if (res == null) {
						res = ProfilePlaceholder.tryFindResolution(req.getState().getFor(ptype), mind.getCulture(),
								mind.getRememberedProfiles(), ptype.isTypePlaceholder());
					}
					if (res != null) {
	...
	public RequirementWrapper chooseNextAction(Mind mind, Iterable<Action> possible, int n) {
		...
		if (getNextGoal().goalUpdate(mind))
			throw new IllegalStateException(
					"Cannot choose a new action; the task is already complete (previous action is "
							+ (!this.isEmpty() ? this.getFirstAction().action : "<none>") + ")");

		PossibleActionWrapper nextAction = this.decideAction(this.getPossibleNextActions(mind, possible, n));
		if (nextAction == null) {
			return null;
		}
		this.tryResolveProfiles(nextAction.reqs, mind);
		Goal goal = null;
		...
		if (this.actions.isEmpty()) {
			...
			if (goal.isComplete())
				throw new IllegalStateException("Previous action " + actions.peek().action + " has no requirements: "
						+ actions.peek().requirement.goalReport());
	...
	public static class ActionWrapper {
		...
		public ActionWrapper(Action action, State requirement, Task task) {
			this(action, new RequirementGoal(requirement, Priority.X_PRIORITY), task);
		}
