...
public class WorldGraphics extends PApplet {
	...
	public void keyPressed(processing.event.KeyEvent event) {
		if (event.getKeyCode() == KeyEvent.VK_W) {
			...
			if (this.currentDisplay == Display.WORLD) {
				UpgradedGroup group = world.getTestGroup();
				System.out.println(group.report());
			} else if (this.currentScreen instanceof UpgradedMindDisplay md) {
	...
	public void mouseClicked(MouseEvent event) {
		...
		if (event.getButton() == RIGHT) {
			/* if (this.currentDisplay == Display.WORLD) */ if (event.getX() < world.getWidth() + BORDER) {
				Actor a;
				world.spawnActor(a = new UpgradedPerson(world, "baba" + event.getX() + "" + event.getY(), Species.IMP,
						world.clampX(event.getX() - BORDER), world.clampY(event.getY() - BORDER), 5));
				a.setOptionalColor(Color.RED.getRGB());
				((UpgradedPerson) a).getMind().establishRelationship(world.getTestGroup(), Relationship.be());
			...
			/* if (this.currentDisplay == Display.WORLD) */
			{
				...
				if (l != null) {
					...
					if (l instanceof UpgradedSentientActor usa) {
