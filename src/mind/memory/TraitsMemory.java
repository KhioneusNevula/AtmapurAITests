 ...
 *
 */
public class TraitsMemory implements IMeme {
	 ...
	 * @param prof
	 */
	public void acceptProperties(TraitsMemory prof) {
		TraitsMemory.Traits genT = prof.generalTraits;
		this.generalTraits.learnTraits(genT);
		for (Map.Entry<IComponentType, TraitsMemory.Traits> entry : prof.traits.entrySet()) {
			this.getOrInitTraits(entry.getKey()).learnTraits(entry.getValue());
		}
		for (Map.Entry<IComponentPart, TraitsMemory.Traits> entry : prof.specificTraits.entrySet()) {
			this.getOrInitTraits(entry.getKey()).learnTraits(entry.getValue());
		}
	}
	...
	public class Traits {
		 ...
		 * @return
		 */
		public TraitsMemory.Traits convertActorProperties(TraitsMemory oth) {
			TraitsMemory.Traits newTraits = oth.new Traits();
