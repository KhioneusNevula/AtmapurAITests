...
public class CompleteGoalThought extends AbstractThought {
	...
	public CompleteGoalThought(ITaskGoal mainGoal) {
		this.goal = mainGoal;
	}
	...
	public void thinkTick(ICanThink memory, int ticks, long worldTick) {
		switch (phase) {
		case PLANNING:
			ITaskGoal goal = this.actions.empty() ? this.getGoal()
					: (ITaskGoal) this.actions.peek().getPendingCondition(memory);
			if (goal == null) {
				return;
			} else {
				boolean complete = goal.isComplete(memory);
		...
		case ACTING:
			if (this.actions.isEmpty()) {
				...
				if (this.current == null) {
					IActionThought latest = actions.peek();
					latest.notifyShouldResume();
					latest.start();
					this.current = latest;
				}
			}
	...
	public boolean equivalent(IThought other) {
		return other instanceof CompleteGoalThought && this.goal.equivalent(other.getGoal());
	}
