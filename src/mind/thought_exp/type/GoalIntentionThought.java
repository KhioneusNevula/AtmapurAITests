...
public class GoalIntentionThought extends AbstractThought {
	...
	private Map<IActionType<?>, Integer> failedActions = new TreeMap<>(
			(a, b) -> a.getUniqueName().compareTo(b.getUniqueName()));

	public GoalIntentionThought(ITaskGoal goal) {
		this.goal = goal;
	}
	...
	public Interest shouldBecomeMemory(ICanThink mind, int finishingTicks, long worldTicks) {
		return Interest.FORGET; // TODO remember completing goals
	}
	...
	public boolean isFinished(ICanThink memory, int ticks, long worldTick) {
		return complete || failures > maxFailures;
	}
	...
	public void thinkTick(ICanThink memory, int ticks, long worldTick) {
		if (checkCompletion) {
			complete = this.goal.isComplete(memory);
		}
		if (!complete) {
			if (this.childThoughts(ThoughtType.PLAN_ACTION).isEmpty()) {
				this.postChildThought(new PlanActionThought(getGoal()).addFailedActions(this.failedActions.keySet()),
						ticks);
			} else {
	...
	public void getInfoFromChild(IThought childThought, boolean interrupted, int ticks) {
		if (childThought instanceof PlanActionThought at) {
			if (at.succeeded()) {
			} else {
				failures++;
